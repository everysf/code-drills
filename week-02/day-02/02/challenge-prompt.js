...
alert("There are " + elevens + " eleven(s) in the array.");
...
console.log("sumNumArray = " + sumNumArray);
...
for (var i = 0; i < numArray.length; i++) {
  ...
  if (numArray[i] > 50) {
    greaterThanFifty = greaterThanFifty + numArray[i];
  }
...
for (var i = 0; i < numArray.length; i++) {
  ...
  if (numArray[i] != 0) {
    numArrayEven = numArrayEven + numArray[i];
  }

};

console.log("numArrayEven = " + numArrayEven);
...
uniqueFruits = Array.from(new Set(fruits));
